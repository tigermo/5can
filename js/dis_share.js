...
function cbError(opid,errCode,errDes)
{
	if(opid=="30" || opid=="31")
	{
		...
		uexWindow.toast("0","5","分享 Failed",2000);
	}
...
function sharestart(cont, uid, sub, wname){
	...
	uexWindow.actionSheet('', 'Cancel', array);
}
	...
	if(Int(opId)==30)
	{
		...
		if(Int(status)==1)
		{
			...
			if(type==1){
				...
				{
					uexWindow.toast("0","5","分享到新浪微博 Succeed",2000);
				}
				else if(json.error_code==21327)
				{
					...
					uexWindow.toast("0","5","您Already 分享过一次，不能重复分享",2000);
				}
				else
				{
					uexWindow.toast("0","5","分享到新浪微博 Failed",2000);
				}
			}
			else if(type==0){
				var json = JSON.parse(data);
				var str = "分享到腾讯微博 Failed"
				if(json.ret==0) str = "分享到腾讯微博 Succeed";
				...
				var str = "分享到网易微博 Failed"
				if(json.created_at) str = "分享到网易微博 Succeed";
				uexWindow.toast("0","5",str,2000);
			}
			...
			var note = "已分享到新浪微博";
			if (type == 1) {
				if (data == 'error:403') note = '分享 Failed！access_token错误。';
				uexXmlHttpMgr.close(opId);
			}
