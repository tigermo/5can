var zy_json_opid = parseInt(Math.random()*1000);
				...
				if(a.substring(0,4).toLowerCase()=="http")
				{
					uexXmlHttpMgr.onData = function (_opid, status, result)
					{
						...
						else if(result.indexOf('?(')>=0)
						{
							...
							if(str=='') str = '{"status":"-3", "message":"Nothing！"}';
						}
						...
						if(c.toLowerCase()=="json"){
							try{
								...
								str = '{"status":"-2", "message":"json parse failed!"}';
								str = JSON.parse(str);
								uexXmlHttpMgr.close(_opid);
								if(!(zy_json_session[_opid].errcb===undefined))
									zy_json_session[_opid].errcb(str);
							...
							json = result;
						if(zy_json_session[_opid].sd && window.localStorage)
						{
					...
					zy_json_session[zy_json_opid].sd=h;
					//logs('zy_json-->opid='+zy_json_opid+', url='+a);
