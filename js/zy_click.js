	...
	var $E = {
		...
		cancel: $support.touch ? 'touchcancel' : 'touchcancel',
        ...
        _touchStart: function(event) {
            ...
            if (self.css && !self.touchTime) {
                self.touchTime=setTimeout(function(){
					logss('click--->touchTime--->_click='+self._click);
                    if (self._click && self.element.className.indexOf(self.css)<0) 
                        self.element.className += (" " + self.css);
                }, 20);
			}
			
			if(self.touchTime2) clearTimeout(self.touchTime2);
        },
        _touchMove: function(event) {
        	logss('click--->_touchMove--->');
            var self = this;
			if(!self.start)
				return;
           ...
           if((Math.abs(deltaX)>10 || Math.abs(deltaY)>10) && self._click)
           {
